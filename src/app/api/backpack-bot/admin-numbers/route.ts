...
export async function GET(request: NextRequest) {
  try {
    ...
    const adminNumbers = await prisma.backpackAdminNumber.findMany({
      orderBy: [{ isActive: 'desc' }, { phoneNumber: 'asc' }]
    })

    return NextResponse.json({ adminNumbers })
  } catch (error) {
    console.error('Error listando números admin mochilas:', error)
...
export async function POST(request: NextRequest) {
  try {
    ...
    const adminNumber = await prisma.backpackAdminNumber.create({
      data: {
        phoneNumber,
        label: label || null,
    ...
    console.error('Error creando número admin mochila:', error)
    return NextResponse.json({ error: 'Error interno del servidor' }, { status: 500 })
  }
}
