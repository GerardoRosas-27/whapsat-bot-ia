...
export async function PATCH(
  ...
  try {
    ...
    const adminNumber = await prisma.backpackAdminNumber.update({
      where: { id },
      data
    })
    ...
    console.error('Error actualizando número admin mochila:', error)
...
export async function DELETE(
  ...
  try {
    ...
    await prisma.backpackAdminNumber.delete({ where: { id } })
    return new NextResponse(null, { status: 204 })
  } catch (error) {
    console.error('Error eliminando número admin mochila:', error)
    return NextResponse.json({ error: 'Error interno del servidor' }, { status: 500 })
  }
}
