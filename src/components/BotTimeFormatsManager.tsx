...
export default function BotTimeFormatsManager() {
  const [formats, setFormats] = useState<TimeFormat[]>([])
  const [loading, setLoading] = useState(true)
  const [showForm, setShowForm] = useState(false)
  const [editingFormat, setEditingFormat] = useState<TimeFormat | null>(null)
  ...
  const fetchFormats = async () => {
    try {
      ...
      const response = await fetch('/api/bot/patterns?type=time', {
        headers: {
  ...
  const handleEdit = (format: TimeFormat) => {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const token = localStorage.getItem('token')
      const response = await fetch(`/api/bot/patterns/${id}?type=time`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
  ...
  const handleToggleActive = async (format: TimeFormat) => {
    try {
      const token = localStorage.getItem('token')
      ...
      {showForm && (
        <div style={{
          ...
          <h3 style={{ marginBottom: '20px', color: '#333' }}>
            {editingFormat ? 'Editar Formato' : 'Nuevo Formato de Hora'}
          </h3>
